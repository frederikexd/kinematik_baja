...
def build_handover_markdown(store: ProjectStore,
    ...
    L.append("---")
    L.append("_Generated by the KinematiK Baja suspension & integration studio._")
    return "\n".join(L)
...
#  instructions). One document per subsystem keeps the team's docs drawer matching
...
def build_saad_markdown(store: ProjectStore, team_key: str,
    ...
    # ---- Cover sheet ------------------------------------------------------- #
    L.append(f"# {label}")
    L.append("_Society of Automotive Engineers · KinematiK Baja_")
    ...
    ], seed=[f"This document covers the **{label}** subsystem of the "
             f"KinematiK Baja vehicle (part number {part_no})."])
    ...
    L.append(f"_SAAD document for {label} · part {part_no} · generated by the "
             "KinematiK Baja suspension & integration studio. This internal "
